...
def extract_features(url):
    try:
        ...
        return {
            ...
            'is_suspicious_tld':    1 if domain.split('.')[-1] in {'tk','ml','ga','cf','gq','xyz','top','click','yachts'} else 0,
            'has_phishing_keyword': 1 if any(k in url.lower() for k in ['login','verify','account','secure','banking','password','update','confirm']) else 0,
            'is_short_url':         1 if any(s in domain for s in ['bit.ly','tinyurl','t.co','goo.gl','han.gl','c11.kr','nuly.do']) else 0,
            'has_brand_spoofing':   1 if any(b in domain.split('.')[0].lower() for b in ['paypal','apple','google','microsoft','amazon','naver','kakao']) else 0,
            'has_korean_smishing':  1 if re.search(r'택배|배송|결제|환급|당첨|무료|인증|지원금|재난', url) else 0,
            'url_entropy':          -sum((url.count(c)/len(url)) * math.log2(url.count(c)/len(url)) for c in set(url)) if url else 0,
        }
    except:
        return {}
...
def get_suspicious_reasons(features):
    ...
    if features.get('is_suspicious_tld'):    reasons.append("🟠 의심 TLD 사용 (.tk .xyz 등)")
    ...
    if features.get('url_entropy', 0) > 4.5:reasons.append("🟡 URL 복잡도 높음 (랜덤 문자열)")
...
with tab1:
    ...
    with col1:
        check_btn = st.button("🔍 검사하기", type="primary", use_container_width=True)

    # 예시 버튼들
    st.markdown("**빠른 예시 테스트:**")
    ex_col1, ex_col2, ex_col3, ex_col4 = st.columns(4)
    ex1 = ex_col1.button("피싱 예시 1")
    ex2 = ex_col2.button("피싱 예시 2")
    ex3 = ex_col3.button("정상 예시 1")
    ex4 = ex_col4.button("정상 예시 2")

    if ex1: url_input = "http://naver-login.tk/verify/account?user=test"
    if ex2: url_input = "http://kb-bank-secure.xyz/auth/login"
    if ex3: url_input = "https://www.naver.com"
    if ex4: url_input = "https://www.kbstar.com"

    if (check_btn or ex1 or ex2 or ex3 or ex4) and url_input:
        ...
        if result:
            ...
            with st.expander("📊 상세 피처 분석"):
                ...
                f_col1.metric("특수문자 수", feat.get('dot_count',0) + feat.get('hyphen_count',0))
                f_col2.metric("숫자 비율",   f"{feat.get('digit_ratio',0):.2f}")
                f_col3.metric("URL 엔트로피",f"{feat.get('url_entropy',0):.2f}")
