...
def extract_features(url):
    try:
        ...
        return {
            ...
            'has_phishing_keyword': 1 if any(k in domain for k in ['login','verify','secure','banking','account-']) else 0,
            'is_short_url':         1 if any(s in domain for s in ['bit.ly','tinyurl','t.co','goo.gl','han.gl','c11.kr','nuly.do']) else 0,
            'has_brand_spoofing':   1 if any(
                b in domain and not domain.endswith(f'{b}.com') and
                not domain.endswith(f'{b}.co.kr') and
                not domain.endswith(f'{b}.net')
                for b in ['paypal','apple','google','microsoft','amazon','naver','kakao']
            ) else 0,
            'has_korean_smishing':  1 if re.search(r'택배|배송|결제|환급|당첨|무료|인증|지원금|재난', url) else 0,
            'domain_entropy':       -sum((domain.count(c)/len(domain)) * math.log2(domain.count(c)/len(domain)) for c in set(domain)) if domain else 0,
        }
    except:
        return {}
